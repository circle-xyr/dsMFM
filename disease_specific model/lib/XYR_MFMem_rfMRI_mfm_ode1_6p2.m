function dy = XYR_MFMem_rfMRI_mfm_ode1_6p2(y,parameter,SC,Myelin,atrophy)

% dy = CBIG_MFMem_rfMRI_mfm_ode1_4p(y,parameter,SC)
% dy = CBIG_mfm_rfMRI_mfm_ode1_4p(y,parameter,SC)
%-----------------------------------------------------------------------------
% dy = CBIG_mfm_rfMRI_mfm_ode1(y,parameter,SC)
%
...
G = parameter(5);
w=zeros(size(SC,1),1);
I0 = zeros(size(SC,1),1);
w=parameter(1)-Myelin*parameter(2)-atrophy*parameter(3);
% disp('w=parameter(1)+Myelin*parameter(2)-atrophy*parameter(3);');
I0(1:size(SC,1))=parameter(4);
